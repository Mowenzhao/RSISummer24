...
def get_colormaps(path=config.COLORMAP_PATH):
    ...
    # Adjusted line: Filter out unwanted directories explicitly, including .ipynb_checkpoints
    cmap_directories = list(sorted(
        [d for d in os.listdir(os.path.join("home/mowenzha/class_public/external/RealSpaceInterface/static", path))
         if d in order and not d.startswith('.') and os.path.isdir(os.path.join("home/mowenzha/class_public/external/RealSpaceInterface/static", path, d))],
        key=lambda d: order[d]
    ))
    for directory in cmap_directories:
        categories.append(directory)
        maps_for_category = []
        for cmap in os.listdir(os.path.join("home/mowenzha/class_public/external/RealSpaceInterface/static", path, directory)):
            # Ignore files starting with '.', including .ipynb_checkpoints within directories
            if not cmap.startswith('.'):
...
class DataConnection(tornado.websocket.WebSocketHandler):
    ...
    def send_frame(self, redindex):
        ...
        real = {quantity: base64.b64encode(data.astype(np.float32)) for quantity, data in Valuenew.items()}
        transfer = {quantity: base64.b64encode(data.astype(np.float32)) for quantity, data in TransferData.items()}
        self.write_message(
            json.dumps({
                'type': 'data',
                'progress': progress,
